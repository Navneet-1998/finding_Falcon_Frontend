  ...
  const validator = () => {
    ...
    for (let i = 0; i < 4; i++) {
      if (planetList[i] !== undefined && vehicleList[i] === undefined) {
        enqueueSnackbar("Please Select Vehicle For Planet.", {
          variant: "error",
        });
        return false;
    ...
    if (count > 3) {
      enqueueSnackbar("Please Select Planets And Vehicles.", {
        variant: "error",
      });
      return false;
    }
  ...
  return (
    <>
      <div className="main">
        ...
        {showSuccessModal ? (
          <div>
            <h5 className="my-3">{gotMessage}</h5>
            <h5>Time Taken: {timetaken}</h5>
            <h5>Planet Found:{foundPlanet}</h5>
